...
# Анализ изменений после оптимизации `combined_loader`

## Описание изменений в результатах после компиляции с ключом оптимизации

Программа `combined_loader` была скомпилирована с флагом оптимизации, что должно было улучшить производительность. Для анализа изменений в результате повторного тестирования были предоставлены новые графики.

---

### Эксперимент 1: Влияние количества повторений на пропускную способность

#### График: `output35.png` - Пропускная способность vs Количество повторений

![Image 4](./output35.png)

- **Изменения:**
  - Пропускная способность теперь увеличивается быстрее на меньших значениях числа повторений.
  - Максимальная пропускная способность выше по сравнению с предыдущими тестами, особенно заметно в области насыщения после 10⁵ повторений.
  - Логарифмический рост стал более линейным, указывая на улучшенную масштабируемость и снижение накладных расходов.

- **Заключение:** Оптимизация уменьшила накладные расходы и позволила более эффективно использовать ресурсы, обеспечивая более высокую пропускную способность на всех уровнях числа повторений.

---

### Эксперимент 2: Влияние числа параллельных экземпляров на пропускную способность

#### График: `output36.png` - Пропускная способность vs Количество параллельных экземпляров
![Image 4](./output36.png)

- **Изменения:**
  - Пропускная способность при малом числе экземпляров (до 8) увеличилась на 10-15% по сравнению с не оптимизированной версией.
  - Точка насыщения (около 8 экземпляров) теперь достигается при чуть более высокой пропускной способности.
  - Падение производительности при увеличении числа экземпляров после точки насыщения стало менее выраженным, что указывает на лучшую балансировку нагрузки.

- **Заключение:** Программа лучше справляется с увеличением параллельных экземпляров, эффективно распределяя ресурсы и демонстрируя меньшую конкуренцию за них.
...
1. **Повышение пропускной способности:**
   - На всех уровнях нагрузок (количество повторений и параллельных экземпляров) наблюдается рост пропускной способности.
   - Оптимизация устранила часть накладных расходов, связанных с вводом-выводом и вычислениями, что позволило достичь более высокой производительности.

2. **Масштабируемость:**
   - Линейный рост пропускной способности при увеличении повторений стал более выраженным.
   - Увеличение числа параллельных экземпляров теперь приводит к меньшему снижению производительности из-за конкуренции за ресурсы.

3. **Рекомендации:**
   - Оптимизированная версия демонстрирует лучшую производительность, особенно при высоких нагрузках. Рекомендуется использовать эту конфигурацию для реальных сценариев.
   - Для дальнейшего улучшения можно исследовать влияние других ключей оптимизации и методов распределения нагрузки.

---

### Примечания
- Графики `output35.png` и `output36.png` отражают данные, полученные после применения флага оптимизации.
- Все тесты проводились в условиях, аналогичных предыдущим, для обеспечения сопоставимости результатов.
